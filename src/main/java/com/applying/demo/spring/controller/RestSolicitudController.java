...
public class RestSolicitudController {
	...
	@RequestMapping(value="/solicitud/persona/{id}", method = RequestMethod.GET , produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<ResultMessage> obtenerSolicitudPorPersona(@PathVariable("id") Long personaId, Locale objLocale) throws Exception{
		ResultMessage response = new ResultMessage(SucessMessageEnum.MESSAGE_GENERAL_SUCCESS);
		response.setData(solicitudService.obtenerSolicitudPorPersona(personaId));
		return new ResponseEntity<ResultMessage>(response,SucessMessageEnum.MESSAGE_GENERAL_SUCCESS.getHttpStatus());
	}
	...
	public ResponseEntity<ResultMessage> eliminarSolicitud(@PathVariable("id") Long solicitudId, Locale objLocale) throws Exception{
